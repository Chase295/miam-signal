...
WS_PORT = int(os.getenv("WS_PORT", "8080"))  # WebSocket Gateway Port
WS_PATH = os.getenv("WS_PATH", "/ws/external")

# Geräte-Informationen  
DEVICE_NAME = os.getenv("DEVICE_NAME", "signal-device")  # Name des Gerätes

# Globaler API Key (aus .env oder docker-compose.yml)
API_KEY = os.getenv("SIMPLE_API_KEY", "default-api-key-123")

# Signal-Konfiguration
SIGNAL_SERVER_URL = os.getenv("SIGNAL_SERVER_URL", "signal.local.chase295.de")  # Signal-Server (ohne https://)
SIGNAL_RECEIVE_NUMBER = os.getenv("SIGNAL_RECEIVE_NUMBER", "+4915122215051")  # Eigene Signal-Nummer (für Empfang)
SIGNAL_SEND_NUMBER = os.getenv("SIGNAL_SEND_NUMBER", "+4915122215051")     # Eigene Signal-Nummer (für Senden)
SIGNAL_RECIPIENT_NUMBER = os.getenv("SIGNAL_RECIPIENT_NUMBER", "+4917681328005")  # Standard-Zielnummer (für Senden)

# SSL-Konfiguration (für selbst-signierte Zertifikate)
# Konvertiert String "True"/"False" zu Boolean
SIGNAL_VERIFY_SSL = os.getenv("SIGNAL_VERIFY_SSL", "False").lower() in ('true', '1', 't')

# Signal-URLs (dynamisch aus obigen Variablen gebaut)
